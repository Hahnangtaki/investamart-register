...
public class AccountRegisterResource {
    ...
    public List<AccountRegisterDTO> getAllAccountRegisters(@RequestParam(required = false) String filter) {
        if ("accountindividuregister-is-null".equals(filter)) {
            log.debug("REST request to get all AccountRegisters where accountIndividuRegister is null");
            return accountRegisterService.findAllWhereAccountIndividuRegisterIsNull();
        }
        if ("accountinstitutionregister-is-null".equals(filter)) {
            log.debug("REST request to get all AccountRegisters where accountInstitutionRegister is null");
            return accountRegisterService.findAllWhereAccountInstitutionRegisterIsNull();
        }
